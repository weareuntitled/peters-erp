...
const AngebotDetailPage = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [showSendModal, setShowSendModal] = useState(false);

  const { data: angebot, isLoading: isDocLoading } = useQuery({
  ...
  const { data: kunde, isLoading: isKundeLoading } = useQuery({
    ...
    enabled: !!angebot?.kunde_id,
    queryFn: async () => {
      const res = await apiClient.get(`/kunden/${angebot.kunde_id}`);
      return res.data;
    },
  });
  ...
  if (!angebot) {
    return <div className="p-6 text-red-500">Angebot nicht gefunden</div>;
  }
  ...
  const statusColor = (status: string) => {
    ...
    switch (s) {
      ...
      case 'akzeptiert': return 'bg-emerald-100 text-emerald-800';
      case 'abgelehnt': return 'bg-red-100 text-red-800';
      default: return 'bg-zinc-100 text-zinc-800';
    }
  };

  const label = typeLabels[angebot.typ] || 'Dokument';
    ...
    <div className="mx-auto flex max-w-[1200px] flex-col gap-6">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            <h1 className="font-heading text-2xl font-bold text-slate-900">
              {angebot.typ} #{angebot.dokument_nr}
            </h1>
            <span className={`rounded px-2 py-1 text-xs font-semibold uppercase tracking-wider ${statusColor(angebot.status)}`}>
              {angebot.status || 'Offen'}
            </span>
          </div>
          <p className="text-sm text-slate-500">Erstellt am {angebot.datum}</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(`/angebote/${id}/edit`)}
          ...
          >
            <ArrowPathIcon className="h-4 w-4" /> In Rechnung umwandeln
          </button>
          <button
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-1 hidden w-48 rounded border border-slate-200 bg-white py-1 shadow-lg group-hover:block z-10">
                <button
                  onClick={handleConvertToInvoice}
                  className="flex w-full items-center px-4 py-2 text-left text-xs text-emerald-600 hover:bg-emerald-50"
                >
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <span className="font-heading text-xl font-bold text-slate-900">{formatCurrency(angebot.betrag_brutto || 0)}</span>
        </div>
        <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Gültig bis</span>
          <span className="font-heading text-xl font-bold text-slate-900">{angebot.gueltig_bis || '—'}</span>
        </div>
        <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Belegdatum</span>
          <span className="font-heading text-lg font-semibold text-slate-900">{angebot.datum}</span>
        </div>
        <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Liefertermin</span>
          <span className="font-heading text-lg font-semibold text-slate-900">{angebot.liefertermin || '—'}</span>
        </div>
      </div>
        ...
        <div className="col-span-8 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="flex flex-col gap-4">
              <h3 className="border-b border-slate-100 pb-2 font-heading text-sm font-bold uppercase tracking-wide text-slate-900">Kunde</h3>
              <div className="flex flex-col gap-1 text-sm text-slate-600">
                <strong className="text-base font-semibold text-slate-900">{kunde?.name || `Kunde #${angebot.kunde_id}`}</strong>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-900">{angebot.auftragsbezeichnung || '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Angebotsnr.:</span>
                  <span className="font-semibold text-slate-900">{angebot.dokument_nr}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex justify-end bg-slate-50 px-6 py-6 border-t border-slate-200">
              <div className="w-64 flex flex-col gap-2 text-sm">
                <div className="flex justify-between text-slate-500">
                  <span>Netto</span>
                  <span>{formatCurrency(angebot.betrag_netto || 0)}</span>
                </div>
                <div className="flex justify-between text-slate-500">
                  <span>MwSt. (19%)</span>
                  <span>{formatCurrency((angebot.betrag_brutto || 0) - (angebot.betrag_netto || 0))}</span>
                </div>
                <div className="flex justify-between border-t border-slate-200 pt-2 mt-2 font-heading text-lg font-bold text-slate-900">
                  <span>Brutto</span>
                  <span>{formatCurrency(angebot.betrag_brutto || 0)}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="relative border-l-2 border-slate-100 ml-3 space-y-8">
              <div className="relative pl-6">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-slate-400">{angebot.datum} am System</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* PDF Fehlt Indicator */}
      {!angebot.gedruckt && !angebot.gemailt && (
        <div className="flex items-center gap-3 rounded-xl bg-amber-50 p-4 text-sm text-amber-800 border border-amber-200">
          ...
          <span>Dieses Angebot wurde noch nicht gedruckt oder versendet.</span>
        </div>
      )}
      ...
      {showSendModal && (
        <SendDocumentModal
          ...
          dokument={{
            id: angebot.id,
            typ: angebot.typ,
            dokument_nr: angebot.dokument_nr,
...
export default AngebotDetailPage;
