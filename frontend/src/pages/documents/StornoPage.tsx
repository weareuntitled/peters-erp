...
const StornoPage = () => {
  ...
  const [limit, setLimit] = useState(() => {
    const stored = localStorage.getItem('gswin_limit_storno');
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['storno', search, page, limit, sort, dir],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['storno'] });
      setDeleteId(null);
    },
  });
  ...
  const handleLimitChange = (newLimit: number) => {
    setLimit(newLimit);
    setPage(1);
    localStorage.setItem('gswin_limit_storno', String(newLimit));
  };

  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-end justify-between">
        <h1 className="font-heading text-base font-semibold text-sky-950">Stornierungen</h1>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1 rounded bg-white p-4 outline outline-1 outline-slate-200">
          <input
            ...
            placeholder="Stornierung suchen..."
          ...
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              ...
              <th className="px-6 py-3 text-left">
                <SortableHeader label="STORNO" field="dokument_nr" currentSort={sort} currentDir={dir} onSort={toggleSort} />
              </th>
              ...
              <th className="w-24 px-6 py-3 text-right">AKTIONEN</th>
            </tr>
          </thead>
          <tbody>
            {data?.items?.map((doc: any) => (
              <tr
                ...
                onClick={() => navigate(`/storno/${doc.id}`)}
              >
                ...
                <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                  <div className="flex items-center justify-end gap-1">
                    <button
                      onClick={() => navigate(`/storno/${doc.id}/edit`)}
            ...
            {(!data?.items || data.items.length === 0) && (
              <tr><td colSpan={6} className="px-6 py-8 text-center text-sm text-slate-500">Keine Stornierungen gefunden</td></tr>
            )}
          </tbody>
        </table>
        ...
        {data && data.pages > 1 && (
          <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-6 py-4">
            <span className="text-xs font-semibold tracking-tight text-slate-500">
              Zeige {(data.page - 1) * data.size + 1}-{Math.min(data.page * data.size, data.total)} von {data.total} Stornierungen
            </span>
      ...
      {deleteId !== null && (
        <DeleteModal
          ...
          title="Stornierung löschen"
          message="Möchten Sie diese Stornierung wirklich löschen?"
        />
      )}
    </div>
  );
